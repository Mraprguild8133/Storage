...
class FileSharingBot:
    ...
    async def start_command(self, update: Update, context: CallbackContext):
        try:
            ...
            storage_type = "☁️ Permanent cloud storage with Wasabi" if self.wasabi.is_available() else "💾 Local file storage"
...
• Send a video → Get streaming link
        ...
        try:
            ...
            try:
                ...
                if self.wasabi.is_available():
                    ...
                    await progress_msg.edit_text("💾 Storing file locally...")
                    wasabi_url = f"file://{file_path}"  # Local fallback
                
                if not wasabi_url:
                    await progress_msg.edit_text("❌ Error storing file.")
                ...
                # Clean up temp file if Wasabi was used
                if self.wasabi.is_available() and os.path.exists(file_path):
                    os.remove(file_path)
    ...
    async def finalize_file_upload(self, update: Update, context: CallbackContext, file_data: Dict):
        try:
            ...
            storage_type = "Wasabi Cloud" if self.wasabi.is_available() else "Local Storage"
...
• Status: {wasabi_stats.get('status', 'Active')}
""" if self.wasabi.is_available() else "💾 **Storage:** Local file storage only"
...
""" if self.wasabi.is_available() else "💾 **Storage:** Local file storage"
...
This action will delete the file from storage and cannot be undone!
            """
    ...
    async def button_callback(self, update: Update, context: CallbackContext):
        try:
            ...
            if data == "start":
                ...
                storage_info = "☁️ **Files are stored in Wasabi Cloud Storage**" if self.wasabi.is_available() else "💾 **Files are stored locally on the server**"
                ...
                if file_data and (file_data['user_id'] == user_id or user_id in self.config.ADMIN_IDS):
                    # Delete from Wasabi first if available
                    if self.wasabi.is_available() and 'wasabi_url' in file_data:
    ...
    def run(self):
        ...
        if self.wasabi.is_available():
            ...
            print("💾 Local file storage (Wasabi not configured)")
